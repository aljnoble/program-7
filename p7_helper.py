...
def solve_randomly(*args):
    ...
    args = list(args) + ["--sign-def=3", "--seed=" + str(random.randint(01 << 30))]
    return solve(*args)
...
def render_ascii_dungeon(design):
    ...
    block = ''.join(
        [''.join([glyph[sprite.get((r, c), 'space')] + ' ' for c in range(width)]) + '\,' for r in range(width)])
    return block
...
def render_ascii_touch(design, target):
    ...
    for cell, state in design['touch']:
        ...
        param = dict(design['param'])
        width = param['width']
        block = ''.join([''.join([str(touch[r, c]) + ' ' for c in range(width)]) + '\,' for r in range(width)])
        return block
